...
if not isinstance(test_roi, pd.Series):
    test_roi = pd.Series(test_roi)
mean_roi_value = test_roi.mean()

plt.subplot(2, 1, 2)
plt.plot(train_roi, label="Train ROI")
plt.plot(test_roi, label="Test ROI")
plt.axhline(mean_roi_value, color='green', linestyle='--', label=f"Mean ROI Value: {mean_roi_value:.4f}")
plt.xlabel("Episode")
plt.ylabel("ROI")
plt.legend()
...
# Calculate mean ROI for test episodes
mean_test_roi = np.mean(test_roi)
...
plt.axhline(mean_test_roi, color='blue', linestyle='--', linewidth=1, label=f"Mean ROI: {mean_test_roi:.4f}")
plt.xlabel("Episode")
plt.ylabel("ROI")
plt.legend()
...
# Annotate mean ROI
plt.text(len(test_roi) - 1, mean_test_roi, f"{mean_test_roi:.4f}", color='blue', fontsize=10, ha='right')

plt.show()

def handle_exponential_data_and_mean(values):
    # Ensure values is a Pandas Series
    if not isinstance(values, pd.Series):
        values = pd.Series(values)

    # Log-transform positive and negative values directly
    log_transformed_values = pd.Series(index=values.index)
    log_transformed_values.update(np.log1p(values[values > 0]))
    log_transformed_values.update(-np.log1p(np.abs(values[values < 0])))

    # Handle NaN values by replacing with the median of the transformed negative values
    log_transformed_values.fillna(log_transformed_values[values < 0].median(), inplace=True)

    # Calculate the mean of the transformed values
    mean_log_value = log_transformed_values.mean()

    return log_transformed_values, mean_log_value


# Ensure test_roi is a Pandas Series before passing it to the function
if not isinstance(test_roi, pd.Series):
    test_roi = pd.Series(test_roi)

# Apply the function to handle exponential data and compute the mean
log_transformed_roi, mean_log_value = handle_exponential_data_and_mean(test_roi)

# Calculate the mean of the original ROI data (not log-transformed)
mean_roi_value = test_roi.mean()

# Plotting the results
plt.figure(figsize=(12, 6))
plt.plot(test_roi, label="Original Test ROI", alpha=0.5, marker='o')
plt.plot(log_transformed_roi, label="Log-Transformed ROI", color='purple', marker='o', linestyle='--')
plt.axhline(mean_roi_value, color='green', linestyle='--', label=f"Mean ROI Value: {mean_roi_value:.4f}")
plt.axhline(mean_log_value, color='blue', linestyle='--', label=f"Mean Log Value: {mean_log_value:.4f}")

# Annotate mean values
plt.text(len(test_roi) - 1, mean_roi_value, f"{mean_roi_value:.4f}", color='green', fontsize=12, ha='right')
plt.text(len(test_roi) - 1, mean_log_value, f"{mean_log_value:.4f}", color='blue', fontsize=12, ha='right')

# Plot details
plt.xlabel("Episode")
plt.ylabel("Value")
plt.title("Log-Transformed ROI with Mean Values")
plt.legend()
plt.grid(True)
plt.show()
